...
namespace Kernel::FS
{
	...
	template <class Y>
	size_t EXT2::expand_indirect_1_indexes(Utils::shared_ptr<block_t> indir, Y& blocks, size_t start, size_t count, bool include_nulls)
	{
		assert(indir);
		if (unlikely(!indir))
		{
	...
	template <class Y>
	size_t EXT2::expand_indirect_2_indexes(Utils::shared_ptr<block_t> indir2, Y& blocks, size_t start, size_t count, bool include_nulls)
	{
		assert(indir2);
		if (unlikely(!indir2))
		{
		...
		while (i < pperblk && j < count)
		{
			...
			{
				j += expand_indirect_1_indexes(get_block(ptrs[i]), blocks, start, count-j, include_nulls);
			}
			start = 0;
			++i;
		}
	...
	template <class Y>
	size_t EXT2::expand_indirect_3_indexes(Utils::shared_ptr<block_t> indir3, Y& blocks, size_t start, size_t count, bool include_nulls)
	{
		assert(indir3);
		if (unlikely(!indir3))
		{
		...
		while (i < pperblk && j < count)
		{
			...
			{
				j += expand_indirect_2_indexes(get_block(ptrs[i]), blocks, start, count-j, include_nulls);
			}
			start = 0;
			++i;
		}
